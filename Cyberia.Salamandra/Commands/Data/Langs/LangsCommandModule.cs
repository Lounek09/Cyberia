...
#pragma warning disable CA1822 // Mark members as static
    ...
    public sealed class LangsCommandModule : ApplicationCommandModule
    {
            ...
            string name)
        {
            ...
            Lang? lang = Bot.Instance.LangsWatcher.GetLangsByType(type).GetLangsByLanguage(language).GetLangByName(name);
            if (lang is null)
            {
            ...
            string? languageStr = null)
        {
            ...
            foreach (Language language in languages)
            {
                tasks.Add(Task.Run(async () =>
                {
                    ...
                    LangsData data = Bot.Instance.LangsWatcher.GetLangsByType(type).GetLangsByLanguage(language);
                    LangsData dataModel = Bot.Instance.LangsWatcher.GetLangsByType(typeModel).GetLangsByLanguage(language);
