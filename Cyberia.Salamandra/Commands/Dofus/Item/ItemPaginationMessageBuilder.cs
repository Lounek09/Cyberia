using Cyberia.Api.DatacenterNS;
using Cyberia.Salamandra.Managers;

using DSharpPlus;
using DSharpPlus.Entities;
using DSharpPlus.EventArgs;

namespace Cyberia.Salamandra.Commands.Dofus
{
    public sealed class ItemPaginationMessageBuilder : PaginationMessageBuilder
    {
        private readonly List<Item> _items;

        public ItemPaginationMessageBuilder(List<Item> items) :
            base(DofusEmbedCategory.Inventory, "Items", "Plusieurs objets trouvés :")
        {
            _items = items;
            PopulateContent();
        }

        protected override void PopulateContent()
        {
            foreach (Item item in _items)
                _content.Add($"- Niv.{item.Level} {Formatter.Bold(item.Name.SanitizeMarkDown())} ({item.Id})");
        }

        protected override DiscordSelectComponent SelectBuilder()
        {
            HashSet<DiscordSelectComponentOption> options = new();
            foreach (Item item in _items.GetRange(GetStartPageIndex(), GetEndPageIndex()))
                options.Add(new(item.Name.WithMaxLength(100), item.Id.ToString(), item.Id.ToString()));

            return new("select", "Sélectionne un item pour l'afficher", options);
        }

        protected override async Task<bool> InteractionTreatment(ComponentInteractionCreateEventArgs e)
        {
            if (await base.InteractionTreatment(e))
                return true;

            if (e.Id.Equals("select"))
            {
                int id = Convert.ToInt32(e.Interaction.Data.Values.First());

                Item? item = Bot.Instance.Api.Datacenter.ItemsData.GetItemById(id);
                if (item is not null)
                {
                    await new ItemMessageBuilder(item).UpdateInteractionResponse(e.Interaction);
                    return true;
                }
            }

            await e.Interaction.CreateResponseAsync(InteractionResponseType.DeferredMessageUpdate);
            return false;
        }
    }
}
