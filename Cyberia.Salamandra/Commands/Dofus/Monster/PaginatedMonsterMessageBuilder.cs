...
public sealed class PaginatedMonsterMessageBuilder : PaginatedMessageBuilder<MonsterData>
{
    ...
    public static PaginatedMonsterMessageBuilder? Create(IServiceProvider provider, int version, CultureInfo? culture, string[] parameters)
    {
        if (version == PacketVersion &&
            parameters.Length > 1 &&
            int.TryParse(parameters[0], out var selectedPageIndex))
        {
            var monstersData = DofusApi.Datacenter.MonstersRepository.GetMonstersDataByName(parameters[1], culture).ToList();
