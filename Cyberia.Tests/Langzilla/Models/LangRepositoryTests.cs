...
public sealed class LangRepositoryTests
{
    ...
    public void Setup()
    {
        _versions = File.ReadAllText(SharedData.VERSIONS_PATH);          
    ...
    {
        var repository = LangRepository.LoadFromFile(SharedData.LANG_REPOSITORY_DATA_PATH);
        ...
        Assert.AreEqual(Path.Join(LangsWatcher.OUTPUT_PATH, "official", "fr"), repository.OutputPath);
        ...
        Assert.AreEqual(Path.Join(LangsWatcher.OUTPUT_PATH, "official", "fr", "versions_fr.txt"), repository.VersionFilePath);
    }
    ...
    {
        var json = File.ReadAllText(SharedData.LANG_REPOSITORY_DATA_PATH);
    ...
    public void GetByName_ValidLangName_ReturnsLang()
    {
        ...
        Assert.AreEqual(Path.Join(LangsWatcher.OUTPUT_PATH, "official", "fr", "items"), lang.OutputPath);
        Assert.AreEqual("items_fr_1184.swf", lang.FileName);
        Assert.AreEqual("lang/swf/items_fr_1184.swf", lang.FileRoute);
        Assert.AreEqual(Path.Join(LangsWatcher.OUTPUT_PATH, "official", "fr", "items", "items_fr_1184.swf"), lang.FilePath);
        Assert.AreEqual(Path.Join(LangsWatcher.OUTPUT_PATH, "official", "fr", "items", "current.as"), lang.CurrentDecompiledFilePath);
        Assert.AreEqual(Path.Join(LangsWatcher.OUTPUT_PATH, "official", "fr", "items", "old.as"), lang.OldDecompiledFilePath);
        Assert.AreEqual(Path.Join(LangsWatcher.OUTPUT_PATH, "official", "fr", "items", "diff.as"), lang.DiffFilePath);
    }
