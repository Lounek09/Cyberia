using Cyberia.Api.Factories.Criteria;
using Cyberia.Api.Factories.EffectAreas;

using System.Globalization;

namespace Cyberia.Api.Factories.Effects.Elements;

public sealed record GladiatroolGiveTokenAtEndFightTonicEffect : Effect
{
    public int CurrentTonicBuy { get; init; }
    public int MaxTonicBuyable { get; init; }
    public int Quantity { get; init; }

    private GladiatroolGiveTokenAtEndFightTonicEffect(int id, int currentTonicBuy, int maxTonicBuyable, int quantity)
        : base(id)
    {
        CurrentTonicBuy = currentTonicBuy;
        MaxTonicBuyable = maxTonicBuyable;
        Quantity = quantity;
    }

    internal static GladiatroolGiveTokenAtEndFightTonicEffect Create(int effectId, EffectParameters parameters)
    {
        return new(effectId, (int)parameters.Param1, (int)parameters.Param2, (int)parameters.Param3);
    }

    public override DescriptionString GetDescription(CultureInfo? culture = null)
    {
        return GetDescription(culture, CurrentTonicBuy, MaxTonicBuyable, Quantity);
    }
}
