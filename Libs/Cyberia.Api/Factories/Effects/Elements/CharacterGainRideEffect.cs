...
public sealed record CharacterGainRideEffect : Effect, IRideEffect, IRideAbilityEffect
{
    public int RideId { get; init; }
    public int RideAbilityId { get; init; }
    public bool Infertile { get; init; }

    private CharacterGainRideEffect(int id, int rideId, int rideAbilityId, bool infertile)
        : base(id)
    {
        RideId = rideId;
    ...
    internal static CharacterGainRideEffect Create(int effectId, EffectParameters parameters)
    {
        return new(effectId, (int)parameters.Param3, (int)parameters.Param2, parameters.Param1 == 1);
    }
