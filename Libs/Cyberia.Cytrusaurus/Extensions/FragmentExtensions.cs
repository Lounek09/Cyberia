...
public static class FragmentExtensions
{
    extension(Fragment fragment)
    {
        ...
        internal IEnumerable<GameFile> GetGameFiles()
        {
        ...
        internal string Diff(Fragment model)
        {
            List<KeyValuePair<int, string>> diff = new();

            var currentGameFiles = fragment.GetGameFiles().ToList();
            ...
            if (diff.Count > 0)
            {
                diff.Add(new KeyValuePair<int, string>(-1, $@"// {fragment.Name.ToUpper()} \\"));
            }
