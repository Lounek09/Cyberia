...
public sealed class EffectReadOnlyListOfReadOnlyListConverter : JsonConverter<IReadOnlyList<IReadOnlyList<Effect>>>
{
    ...
    public override IReadOnlyList<IReadOnlyList<Effect>> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        List<IReadOnlyList<Effect>> effects = [];

        while (reader.Read() && reader.TokenType != JsonTokenType.EndArray)
        {
            effects.Add(_effectReadOnlyListConverter.Read(ref reader, typeof(IReadOnlyList<Effect>), options));
        }

        return effects.AsReadOnly();
    }

    public override void Write(Utf8JsonWriter writer, IReadOnlyList<IReadOnlyList<Effect>> values, JsonSerializerOptions options)
    {
