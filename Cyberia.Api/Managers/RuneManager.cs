...
public static class RuneManager
{
    ...
    public static double GetPercentStatExtractable(this RuneData runeData, int itemLvl, int statAmount)
    {
        ...
        var c = 1.5 * (a / b);
        var percentRuneExtractable = c + (statAmount - 1) / runeData.Weight * Math.Max(0, 2D / 3 * 100 - c);
    ...
    {
        var multiplicator = runeData.GetStatAmountExtractableToObtain(type) / (runeData.GetPercentStatExtractable(itemLvl, statAmount) / 100) / statAmount;
    ...
    public static double[] GetTotalRunesByStatAmontExtractable(this RuneData runeData, double amountExtractable)
    {
        ...
        var lastBaRune = Math.Floor(amountExtractable);
